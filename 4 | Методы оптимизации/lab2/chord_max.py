...
def chord_method_max_history(a, b, eps):
    history = [(a, b)]
    secants = []
    iters = 0

    print(f"Поиск максимума методом хорд на [{a}, {b}] с точностью {eps}\n")
...
eps = 0.001
a0, b0 = 0.0, 0.6
history, secants, x_max = chord_method_max_history(a0, b0, eps)

fig, (ax1, ax2, ax3) = plt.subplots(3, 1, figsize=(12, 16))

x_full = np.linspace(-1, 1.5, 1000)
ax1.plot(x_full, f(x_full), 'k-', label='f(x)')
ax1.axvspan(a0, b0, color='red', alpha=0.15, label=f'Промежуток: [{a0}, {b0}]')
ax1.scatter([x_max], [f(x_max)], color='red', s=80, zorder=5, label='Найденный максимум')
ax1.set_title('Локальный максимум функции f(x)')
ax1.set_xlabel('x')
ax1.set_ylabel('f(x)')
ax1.grid(True, linestyle='--', alpha=0.6)
ax1.legend()

x_zoom = np.linspace(a0 - 0.05, b0 + 0.05, 500)
ax2.plot(x_zoom, df(x_zoom), 'g-', linewidth=2, label="f'(x) (Производная)")
ax2.axhline(0, color='black', linewidth=1)

colors = plt.cm.plasma(np.linspace(0, 0.8, len(secants)))
...
ax2.set_title("Поиск корня производной f'(x)=0 методом хорд (для максимума)")
ax2.set_xlabel('x')
ax2.set_ylabel("f'(x)")
...
for i, (a, b) in enumerate(history):
    ax3.plot([a, b], [i, i], color='red', marker='|', markersize=8, linewidth=2)
    ax3.text(b + 0.01, i, f'Ит. {i}: [{a:.4f}, {b:.4f}]', va='center', fontsize=9)

ax3.set_title('Визуализация сужения отрезка (Метод хорд - Максимум)')
ax3.set_xlabel('x')
ax3.set_ylabel('Номер итерации')
ax3.set_xlim(a0 - 0.05, b0 + 0.15)
ax3.invert_yaxis()
