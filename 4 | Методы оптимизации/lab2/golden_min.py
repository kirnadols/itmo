...
def golden_section_history(a, b, eps, find_max=False):
    history = [(a, b)]
    iters = 0
    ...
    print(f"Поиск минимума на промежутке [{a}, {b}] с точностью {eps}\n")
...
eps = 0.001
a0, b0 = 1.0, 2.0
history, x_min = golden_section_history(a0, b0, eps, find_max=False)

fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(12, 10))

x_full = np.linspace(-5, 3, 2000)
ax1.plot(x_full, f(x_full), 'k-', label='f(x)')
ax1.axvspan(a0, b0, color='blue', alpha=0.15, label=f'Промежуток унимодальности: [{a0}, {b0}]')
ax1.scatter([x_min], [f(x_min)], color='blue', s=80, zorder=5, label='Найденный минимум')

ax1.set_title('Локальный минимум на отрезке [1.0, 2.0]')
ax1.set_xlabel('x')
ax1.set_ylabel('f(x)')
...
ax2.set_title('Визуализация сужения отрезка (Метод золотого сечения)')
ax2.set_xlabel('x')
ax2.set_ylabel('Номер итерации')
ax2.set_xlim(a0 - 0.05, b0 + 0.3)
ax2.invert_yaxis()
