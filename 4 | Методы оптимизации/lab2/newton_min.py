...
def newton_method_history(x0, eps):
    history = [x0]
    tangents = []
    x = x0
    iters = 0

    print(f"Поиск минимума методом Ньютона. Стартовая точка x0 = {x0}, точность {eps}\n")
...
x_start = 1.9
a0, b0 = 1.0, 2.0
history, tangents, x_min = newton_method_history(x_start, eps)

fig, (ax1, ax2, ax3) = plt.subplots(3, 1, figsize=(12, 16))

x_full = np.linspace(-5, 3, 2000)
ax1.plot(x_full, f(x_full), 'k-', label='f(x)')
ax1.axvspan(a0, b0, color='blue', alpha=0.15, label=f'Рассматриваемый промежуток: [{a0}, {b0}]')

for i, x_val in enumerate(history):
    alpha_val = max(0.3, i / len(history))
    ax1.scatter([x_val], [f(x_val)], color='blue', alpha=alpha_val, s=60, zorder=5)

ax1.scatter([x_min], [f(x_min)], color='cyan', edgecolors='black', s=100, zorder=6, label='Найденный минимум')
ax1.set_title('Локальный минимум функции f(x) (Метод Ньютона)')
ax1.set_xlabel('x')
ax1.set_ylabel('f(x)')
ax1.grid(True, linestyle='--', alpha=0.6)
ax1.legend()

x_zoom = np.linspace(1.8, 2.1, 500)
ax2.plot(x_zoom, df(x_zoom), 'g-', linewidth=2, label="f'(x) (Производная)")
ax2.axhline(0, color='black', linewidth=1)

colors = plt.cm.spring(np.linspace(0, 0.8, len(tangents)))
...
ax2.set_title("Поиск корня производной f'(x)=0 касательными")
ax2.set_xlabel('x')
ax2.set_ylabel("f'(x)")
...
ax3.plot(iterations, history, color='blue', marker='o', linestyle='-', linewidth=2, markersize=8)
for i, x_val in enumerate(history):
    ax3.text(i, x_val + 0.005, f'{x_val:.4f}', ha='center', fontsize=9)

ax3.axhline(x_min, color='red', linestyle='--', alpha=0.5, label='Истинный минимум')
ax3.set_title('Траектория приближения к точке минимума $x^*$')
